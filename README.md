...
| 文件 | 描述 |
|------|------|
| `agent.py` | 主智能体图、LLM 配置、交互式 CLI（IAM 凭证认证） |
| `agent_apikey.py` | 同上，使用 Bedrock API Key（Bearer Token）认证 |
...
#### 方式二：Bedrock API Key（`agent_apikey.py`）

```bash
export BEDROCK_API_KEY="your-bedrock-api-key"
```

使用 API Key 时无需配置 IAM 凭证，Bearer Token 会在每次请求前自动注入 `Authorization` 头。
